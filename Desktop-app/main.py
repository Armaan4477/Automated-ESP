...
class ScheduleDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.relay_combo.addItems([f"Relay {i+1}" for i in range(2)])
...
class ESPControl(QMainWindow):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for i, schedule in enumerate(schedules):
                    ...
                    toggle_btn = QPushButton("Deactivate" if schedule["enabled"] else "Activate")
                    toggle_btn.clicked.connect(lambda _, s=schedule: self.toggle_schedule(s))
                    action_layout.addWidget(toggle_btn)
                    
                    delete_btn = QPushButton("Delete")
                    delete_btn.clicked.connect(lambda _, x=schedule["id"]: self.delete_schedule(x))
    ...
    def toggle_schedule(self, schedule):
        try:
            ...
            response = requests.post(
                f"http://{self.esp_ip}/schedule/update",
                json={"id": schedule["id"], "enabled": new_state}
            )
            if response.status_code == 200:
